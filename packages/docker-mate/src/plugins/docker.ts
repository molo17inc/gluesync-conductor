...
const dockerPlugin = async (fastify: FastifyInstance) => {
  try {
    ...
    if (process.env.DOCKER_HOST) {
      fastify.log.info(`Using Docker host from environment: ${process.env.DOCKER_HOST}`);
      docker = new Docker();
      ...
      if (socketPath) {
        ...
        fastify.log.warn('Could not find Docker socket on macOS, using default configuration');
        docker = new Docker();
      }
    } else {
      // Default for Linux and other platforms
      fastify.log.info('Using default Docker socket path: /var/run/docker.sock');
    ...
    fastify.log.error(`Error initializing Docker plugin: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
};
