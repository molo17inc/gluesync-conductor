...
const handler: RouteHandlerMethod = async (req, reply) => {
  try {
    ...
    try {
      ...
      const response = await axios.get(`https://api.backoffice.molo17.com/agent/${name}`);
      ...
      req.log.error(`Error fetching latest version for ${name}: ${apiError instanceof Error ? apiError.message : String(apiError)}`);
    ...
    req.log.error(`Error getting container version: ${error instanceof Error ? error.message : String(error)}`);
