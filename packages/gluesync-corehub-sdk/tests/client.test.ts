...
describe('GluesyncClient', () => {
  ...
  beforeEach(() => {
    ...
    (discovery.getCorehubAddress as jest.Mock).mockResolvedValue(
      '192.168.1.100',
    );
    ...
    (WebSocketConnection.prototype.connect as jest.Mock).mockResolvedValue(
      'jwt-token-123',
    );
  ...
  test('should handle license errors', async () => {
    ...
    await expect(clientWithInvalidLicense.connect()).rejects.toThrow(
      GluesyncLicenseError,
    );
  ...
  test('should emit events', async () => {
    ...
    const connectionInstance = (WebSocketConnection as unknown as jest.Mock)
      .mock.instances[0];
