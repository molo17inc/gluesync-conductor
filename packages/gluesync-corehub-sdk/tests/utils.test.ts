  ...
  describe('createSslContextFromJks', () => {
    beforeEach(() => {
      ...
      jest
        .spyOn(fs, 'readFile')
        .mockImplementation(() =>
          Promise.resolve(Buffer.from('mock-jks-data')),
        );
      ...
      jest
        .spyOn(forge.pki, 'privateKeyFromPem')
        .mockReturnValue('mock-private-key-object' as any);
      jest
        .spyOn(forge.pki, 'publicKeyToPem')
        .mockReturnValue('mock-public-key-pem');
      jest
        .spyOn(forge.pki, 'privateKeyToPem')
        .mockReturnValue('mock-private-key-pem');
    });

    test('should create SSL context from JKS file', async () => {
      // Call the function
      const sslOptions = await createSslContextFromJks(
        'keystore.jks',
        'password',
      );
      ...
      expect(JSZip.loadAsync).toHaveBeenCalledWith(
        Buffer.from('mock-jks-data'),
      );
    ...
    test('should handle errors when reading JKS file', async () => {
      ...
      await expect(
        createSslContextFromJks('keystore.jks', 'password'),
    ...
    test('should handle errors when parsing JKS file', async () => {
      ...
      jest
        ...
        .mockRejectedValue(new Error('Invalid JKS file'));
